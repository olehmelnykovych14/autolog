...
const INIT_CARS = [
  { id:1, brand:'BMW 5 Series', plate:'BM4554AA', mileage:136400, year:2021, status:'ok' },
  { id:2, brand:'Toyota Camry', plate:'KA7777BB', mileage:85000, year:2019, status:'warning' },
...
function Sidebar({ tab, setTab, col, setCol }) {
  return (
    <aside className="flex flex-col h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 shrink-0 transition-all duration-300" style={{width:col?72:260}}>
...
// ─── Topbar ──────────────────────────────────────────────────────────────────
function Topbar({ isDark, setDark }) {
  return (
    <header className="h-16 bg-white/80 dark:bg-gray-900/80 backdrop-blur border-b border-gray-200/50 dark:border-gray-700/50 flex items-center px-6 gap-4 shrink-0">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <button className="relative w-9 h-9 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 transition-all">
          <Bell size={18}/><span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-red-500"></span>
        </button>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-sm ml-1" style={{background:C}}>І</div>
      </div>
    </header>
  )
}
...
function AddCarModal({ onClose, onAdd }) {
  const [f, setF] = useState({ brand:'', year:new Date().getFullYear(), plate:'', mileage:'' })
  const set = k => v => setF(p => ({...p,[k]:v}))
  const submit = e => {
    e.preventDefault()
    if (!f.brand||!f.plate) return
    onAdd({ id:Date.now(), brand:f.brand, year:parseInt(f.year), plate:f.plate.toUpperCase(), mileage:parseInt(f.mileage)||0, status:'ok' })
  }
  const ic = inp_cls()
  return (
    <Modal title="Додати авто" onClose={onClose}>
      <form onSubmit={submit} className="flex flex-col gap-4">
        <Field label="Марка та модель *"><input value={f.brand} onChange={e=>set('brand')(e.target.value)} placeholder="Напр. BMW 5 Series" className={ic} required/></Field>
...
function CarDetailsModal({ car, onClose, onGoService }) {
  const logo = getBrandLogo(car.brand)
  return (
    <Modal title="Деталі автомобіля" onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[['Пробіг',`${fmt(car.mileage)} км`],['Рік',car.year],['Номер',car.plate],['Стан',car.status==='ok'?'Відмінний':'Потребує уваги']].map(([l,v]) => (
...
function ServiceModal({ onClose, onSave, carList, initialData }) {
  const isEdit = !!initialData
  const [f, setF] = useState({
    carId: initialData?.carId || carList[0]?.id || '',
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 p-6 flex flex-col gap-5">
        <h2 className="font-semibold text-gray-900 dark:text-white">Профіль</h2>
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-bold" style={{background:C}}>І</div>
          <button className="text-sm font-medium hover:opacity-70 transition-all" style={{color:C}}>Змінити фото</button>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Ім'я"><input defaultValue="Іван Петренко" className={ic}/></Field>
          <Field label="Email"><input defaultValue="ivan@example.com" type="email" className={ic}/></Field>
          <Field label="Телефон"><input defaultValue="+380 67 123 45 67" className={ic}/></Field>
          <Field label="Місто"><input defaultValue="Київ" className={ic}/></Field>
        </div>
        <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
          <PrimaryBtn onClick={() => alert('Дані успішно збережено ✓')}>Зберегти зміни</PrimaryBtn>
        </div>
      </div>
    ...
    <ThemeCtx.Provider value={isDark}>
      <div className={`flex h-screen w-full font-sans overflow-hidden bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white ${isDark ? 'dark' : ''}`}>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <Topbar isDark={isDark} setDark={setDark}/>
        ...
        {selectedCar && <CarDetailsModal car={selectedCar} onClose={() => setSelectedCar(null)} onGoService={goService}/>}
      </div>
    </ThemeCtx.Provider>
  )
}
