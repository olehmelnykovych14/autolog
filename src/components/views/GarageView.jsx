...
export function GarageView({ carList, onAddCar, onSelectCar, userProfile, onGoPlans }) {
  ...
  const isLimited = userProfile?.plan === 'Free' && carList.length >= 1
            ...
            >
              <div className="h-48 sm:h-52 overflow-hidden relative">
                {car.image ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-indigo-500/10 to-purple-500/10 flex items-center justify-center group-hover:from-indigo-500/20 group-hover:to-purple-500/20 transition-all duration-500">
                    <Plus size={40} className="text-indigo-200 group-hover:text-indigo-300 transition-colors" />
                  </div>
                )}
                ...
                <div className="absolute top-4 right-4 h-10 w-10 rounded-xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm flex items-center justify-center shadow-lg transform translate-y-[-10px] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
...
function AddCarModal({ onClose, onAdd, isLimited, onGoPlans }) {
  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onload = (re) => setF({ ...f, image: re.target.result })
